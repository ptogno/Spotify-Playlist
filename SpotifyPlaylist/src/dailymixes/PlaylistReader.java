...
public class PlaylistReader {
    ...
    private Playlist[] readPlaylistFile(String playlistFileName)
        ...
        while (file.hasNextLine()
            ...
            if (tokens.length < 8) {
                // 2a.TODO: throw corresponding exception, along with a message // PlaceHolder int FIXME
                throw new ParseException("Tokens length is smaller than necessary", 0);
            }
            else if (tokens.length > 8) {
                // 2b.TODO: throw corresponding exception, along with a message
                throw new ParseException("Tokens length is larger than necessary", 0);
            }
                ...
                if (!isInValidPercentRange(minPop, minRock, minCountry) || 
                    ...
                    throw new DailyMixDataException("Invalid Percentage Range");
                }
                // 5.TODO: Instantiate a new Playlist with the values
                // populated above
                Playlist newPlaylist = new Playlist(playlistName, minPop, minRock, minCountry, maxPop, maxRock, maxCountry, maxSongsPerPlaylist);
        ...
        if (playlistCounter < PlaylistCalculator.NUM_PLAYLISTS) {
            ...
            throw new DailyMixDataException("Less than three playlists in the reader");
        }
    ...
    private ArrayQueue<Song> readQueueFile(String songFileName)
        ...
        while (file.hasNextLine()) {
            ...
            if (!isInValidPercentRange(pop, rock, country)) {
                throw new DailyMixDataException("Invalid Percent Range");
            }
    ...
    private boolean isInValidPercentRange(int num1, int num2, int num3) {
        ...
        return num1 >= PlaylistCalculator.MIN_PERCENT && num2 >= PlaylistCalculator.MIN_PERCENT && 
            num3 >= PlaylistCalculator.MIN_PERCENT && num1 <= PlaylistCalculator.MAX_PERCENT &&
            num2 <= PlaylistCalculator.MAX_PERCENT && num3 <= PlaylistCalculator.MAX_PERCENT;
